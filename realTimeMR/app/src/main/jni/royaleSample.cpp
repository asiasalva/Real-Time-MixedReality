...
#define TAG "com.unitn.jroyale.jni"
...
namespace
{
           ...
           // LOGI("max: %d",max);
            ...
            for (i = 0; i < width * height; i++)
            {
                switch(selected_mode){
                    case 0: //Indoor room reconstruction
                        if (data->points.at(i).z < 1.0)
                        {
                            fill[i] = (int)1.0;
                            rawFill[i] = 1.0;
                        } else if (data->points.at(i).z > 4.0)
                        {
                            fill[i] = (int)4.0;
                            rawFill[i] = 4.0;
                        } else{
                        ...
                        break;
                    case 1: // Room scanning, indoor navigation. Change from case 0 by fps number (10 instead of 5)
                        if (data->points.at(i).z < 1.0)
                        {
                            fill[i] = (int)1.0;
                            rawFill[i] = 1.0;
                        ...
                        break;
                    case 2: //3D object reconstruction
                        if (data->points.at(i).z < 0.5)
                        {
                            fill[i] = (int)0.5;
                            rawFill[i] = 0.5;
                        } else if (data->points.at(i).z > 1.5)
                        {
                            fill[i] = (int)1.5;
                            rawFill[i] = 1.5;
                        } else{
                        ...
                        break;
                    case 3: //Medium size object recognition, face reconstruction
                        if (data->points.at(i).z < 0.3)
                        {
                            fill[i] = (int)0.3;
                            rawFill[i] = 0.3;
                        } else if (data->points.at(i).z > 2.0)
                        {
                            fill[i] = (int)2.0;
                            rawFill[i] = 2.0;
                        } else{
                        ...
                        break;
                    case 4: //Remote collaboration, step by step instruction, table-top gaming. Change from case 3 by fps number (35 instead of 25)
                        if (data->points.at(i).z < 0.3)
                        {
                            fill[i] = (int)0.3;
                            rawFill[i] = 0.3;
                    ...
                    case 5:
                        ...
                        break;
                    case 6: //Mixed mode 30/5 fps
                        fill[i] = (int) (((data->points.at (i).z - min) / span) * 255.0f);
                        rawFill[i] = data->points.at (i).z;
                        break;
                    case 7: //Mixed mode 50/5 fps
                        fill[i] = (int) (((data->points.at (i).z - min) / span) * 255.0f);
                        rawFill[i] = data->points.at (i).z;
                        break;
                }
    ...
    public:
        ...
        void writePLY (const std::string &filename, const royale::DepthData *data)
        {
            ...
            outputFile.open ("/storage/emulated/0/Android/data/com.unitn.realtimemr/files/"+filename, std::ofstream::out | std::ofstream::app );
            ...
            else
            {
                ...
                stringStream << "comment Generated by realtimeMR" << std::endl;
    ...
    Java_com_unitn_realtimemr_NativeCamera_semaphoreNotify(JNIEnv *env, jclass type, jboolean notificated) {
    ...
    JNI_OnLoad (JavaVM *vm, void *)
    {
        ...
        {
            jclass tmpClazz = env->FindClass ("com/unitn/realtimemr/NativeCamera$AmplitudeListener");
            if (env->ExceptionCheck ())
            {
                LOGE ("can not find class=[com.unitn.realtimemr.NativeCamera.AmplitudeListener] with path=[com/unitn/realtimemr/NativeCamera$AmplitudeListener]");
                return -1;
            }
            LOGI ("found class=[com.unitn.realtimemr.NativeCamera.AmplitudeListener] with path=[com/unitn/realtimemr/NativeCamera$AmplitudeListener]");
        ...
        // Get the onAmplitudes Method from the AmplitudeListener class.
        {
            ...
            if (env->ExceptionCheck ())
            {
                LOGE ("can not get method=[onAmplitudes] with signature=[([I)V] from class=[com.unitn.realtimemr.NativeCamera.AmplitudeListener]");
                return -1;
            }
            LOGI ("got method=[onAmplitudes] with signature=[([I)V] from class=[com.unitn.realtimemr.NativeCamera.AmplitudeListener]");
        }
    ...
    Java_com_unitn_realtimemr_NativeCamera_openCameraNative (JNIEnv *env, jclass type, jint fd, jint vid, jint pid)
    {
        ...
        int i = 5;
        selected_mode = i;
        ret = cameraDevice->setUseCase (opModes[i]);
        if (ret != royale::CameraStatus::SUCCESS)
        {
    ...
    JNIEXPORT void JNICALL
    Java_com_unitn_realtimemr_NativeCamera_registerAmplitudeListener (JNIEnv *env, jclass type, jobject amplitudeListener)
    {
    ...
    JNIEXPORT void JNICALL
    Java_com_unitn_realtimemr_NativeCamera_closeCameraNative (JNIEnv *env, jclass type)
    {
        cameraDevice->stopCapture ();
    }

    JNIEXPORT jint JNICALL
    Java_com_unitn_realtimemr_NativeCamera_recordRRF(JNIEnv *env, jclass type, jint argc, jstring file_name, jintArray argv)
    {
        LOGI ("JNI RECORDRRF");
        ...
        JNIEXPORT jint JNICALL
        Java_com_unitn_realtimemr_NativeCamera_stopRegistration(JNIEnv *env, jclass type) {
            if (!notified) {
    ...
    JNIEXPORT jint JNICALL
    Java_com_unitn_realtimemr_NativeCamera_convertToPLY(JNIEnv *env, jclass type, jstring file_name) {
    ...
    Java_com_unitn_realtimemr_NativeCamera_getXFocalLength(JNIEnv *env, jclass type) {
        float focalLX  = lens_p.focalLength.first;
        return focalLX;

    }
    JNIEXPORT jfloat JNICALL
    Java_com_unitn_realtimemr_NativeCamera_getYFocalLength(JNIEnv *env, jclass type) {
        float focalLY  = lens_p.focalLength.second;
        return focalLY;
    }
    JNIEXPORT jfloat JNICALL
    Java_com_unitn_realtimemr_NativeCamera_getXFocalCenter(JNIEnv *env, jclass type) {
        float focalCenterX  = x;
        return focalCenterX;
    }
    JNIEXPORT jfloat JNICALL
    Java_com_unitn_realtimemr_NativeCamera_getYFocalCenter(JNIEnv *env, jclass type) {
