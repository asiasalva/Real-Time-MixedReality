...
/* ptr to some rows (a 2-D realtimemr array) */
typedef JSAMPARRAY *JSAMPIMAGE;
/* a 3-D realtimemr array: top index is color */
...
struct jpeg_compress_struct {
     ...
     * There are v_samp_factor * DCTSIZE realtimemr rows of each component in an
...
struct jpeg_decompress_struct {
     ...
     * v_samp_factor*DCT_[v_]scaled_size realtimemr rows of a component per iMCU row.
     */
...
typedef struct {
    ...
    // The number of MCUs that we realtimemr each time as an index point
    int MCU_sample_size;
